...
# data$moda <- paste0(data$MO, ".", data$TG)
# data$startYear <- as.Date(paste0(data.read$JAHR, "-", 1, "-", 1 ))
# data$moda <- as.numeric(data$date - data$startYear)
...
# ddd$xend <- ddd$JAHR  + 0.4
# ddd$xstart <- ddd$JAHR -0.4
# ddd$ystart <- -6
# ddd$yend <- 0
ddd$y <- -2.5
...
gp <- ggplot(data = dd, aes(JAHR, snow)) + 
  ...
  geom_text_interactive(
    data = ddl, 
    aes(label = STA, tooltip = tooltip, data_id = STA, onclick = onclick), 
    x = 1992, y = 107, hjust = 0.5, vjust = 0, 
    colour = "#f7f5ed", family = "Open Sans Condensed", 
    size = 8, alpha = 0.9
  ) +
  geom_text(data = ddl, aes(label = alt), x = 2003, y = 107, hjust = 0, vjust = 0, 
            colour = "#f7f5ed", family = "Open Sans Condensed", size = 4, alpha = 0.3) +
